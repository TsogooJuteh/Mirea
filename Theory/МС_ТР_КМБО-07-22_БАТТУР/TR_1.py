...
def task(X_distribution, name_distribution):
  ...
  if name_distribution == 'binomial' or name_distribution == 'geometric' or name_distribution == 'exponential':
    ...
    if name_distribution == 'exponential':
      num_bins = int(np.round(1 + np.log2(N)))
      intervals = np.linspace(0, X_distribution.max(), num_bins + 1)
  ...
  if name_distribution == 'exponential':
    ...
    counts, bins, patches = ax1.hist(X_distribution, bins=intervals,edgecolor='black', alpha=0.5,weights=np.ones_like(X_distribution) / N,label='Histogram (Relative Frequency)')
    x_line = np.linspace(0, X_distribution.max(), 1000)
    y_line = lamda * np.exp(-lamda * x_line)
    ax1.plot(x_line, y_line, 'r-', label='Theoretical Frequency')
    ax1.set_xlabel('Value')
    ax1.set_ylabel('Relative Frequency')
    ax1.set_title('Histogram (Exponential Distribution)')
    ax1.grid(True)
    ax2 = ax1.twinx()
    ax2.set_ylabel('Count')
    max_rel_freq = counts.max()
    raw_counts_max = max_rel_freq * N
    top_tick = int(math.ceil(raw_counts_max / 20.0)) * 20
    ax2.set_ylim(0, top_tick)
    ax1.set_ylim(0, top_tick / N)
    ax2.set_yticks(range(0, top_tick + 1, 20))
    handles_left, labels_left = ax1.get_legend_handles_labels()
    ax1.legend(handles_left, labels_left, loc='upper right')
  ...
  if name_distribution == 'exponential': 
    ...
    h = intervals[1] - intervals[0]  # new: compute h as bin width from intervals
  ...
  if name_distribution == 'binomial':
    ...
    rel_differences = np.array([abs_differences[i] / abs(theo_values[i]) for i in range(len(theo_values))])
    rel_differences *= 100
